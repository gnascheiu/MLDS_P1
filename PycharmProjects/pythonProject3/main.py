...
class Member():
    ...
    def __init__(self, embed_tf_idf, label=None, doc_id=None):
        self.label = label  # type is int from 0 to (number clusters-1)
        self.doc_id = doc_id  # id of document. type is int
...
class Kmeans():
    ...
    def load_data(self, path_data, vocab_size):
        ...
        for line in list_line:
            ...
            label, id = int(features[0]), int(features[1])
            ...
            # add member
            self.data.append(Member(np.array(embed), label=label, doc_id=id))
