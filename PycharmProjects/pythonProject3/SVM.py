...
def with_linear_SVMS(pathFile, vocab_size):
    ...
    classifier = SVC(C=10.0, tol=0.001, verbose=True)
    classifier.fit(X_train, y_train)
    predict_y = classifier.predict(X_test)
    acc = accuracy(predict_y, y_test)
    return acc

def with_kernel_SVMS(pathFile, vocab_size):
    x, y = load_data(pathFile, vocab_size)
    X_train, X_test, y_train, y_test = train_test_split(
        x, y, test_size=0.33, random_state=42)
...
if __name__ == "__main__":
    ...
    acc_kernel = with_kernel_SVMS("./data_tf_idf.txt", 14140)
    print("accuracy linear SVC ", acc_linear)
    print("accuracy linear SVC ", acc_kernel)
